...
process_http_request(struct tuple4 *addr, u_char *data, int len)
{
	...
	while ((i = buf_index(&buf, "\r\n\r\n", 4)) >= 0) {
		...
		*uri++ = '\0';
		...
		json_object *juser = json_object_new_string((user?user:"-"));
		json_object *jrequest_method = json_object_new_string(req);
		json_object *jrequest_uri = json_object_new_string(uri);
		json_object *jrequest_time = json_object_new_string(timestamp());
		json_object *jhttp_referrer = json_object_new_string((referer?referer:"-"));
		json_object *jhttp_ua = json_object_new_string((agent?agent:"-"));
		json_object *jcookie = json_object_new_string((cookie?cookie:"-"));

		json_object_object_add(log_fields,"source", jsource);
		json_object_object_add(log_fields,"src_addr", jsrc_addr);
		json_object_object_add(log_fields,"dst_addr", jdst_addr);
		json_object_object_add(log_fields,"request_method", jrequest_method);
		json_object_object_add(log_fields,"request_uri", jrequest_uri);
		json_object_object_add(log_fields,"request_time", jreq_time);
		json_object_object_add(log_fields,"http_referrer", jhttp_referrer);
		json_object_object_add(log_fields,"http_user_agent", jhttp_ua);
